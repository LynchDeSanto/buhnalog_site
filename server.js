...
async function verifyTurnstile(token, remoteIp) {
  ...
  if (!secretKey) {
    // Если ключ не настроен, пропускаем проверку (для совместимости)
    return { ok: true };
  }
  ...
  try {
    ...
    if (!result.success) {
      return { ok: false, message: 'Проверка не пройдена. Попробуйте ещё раз.' };
    }

    return { ok: true };
  } catch (error) {
    // При ошибке Cloudflare пропускаем заявку (fail-open режим)
